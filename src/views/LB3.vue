<template>
  <section>
    <div class="container">
      ...
      <section>
        <p>
          Desired result is: {{ desiredResult }} | Result is: {{ result }} |
          Epsilon is: {{ epsilon }}
        </p>
      </section>
      <section>
...
export default defineComponent({
  ...
  setup() {
    const { inputs, teta, desiredResult } = useInputs();
    const { presets } = usePresets();
    const { neuron, epsilon, result, iterations, } = useNeuron(inputs.value, teta.value);
